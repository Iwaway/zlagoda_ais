...
const createMany = async (request, response) => {
    ...
    sales.array.forEach(element => {
        ...
        query += '('+upc+', '+receipt_number+', '+product_number+', '+prices[count].revised_price+'), '
...
const getPercentByCustomer = async (receipt_number) => {
    let percent
    const card_number = await pool.query('SELECT card_number FROM receipt WHERE receipt_number = $1',[receipt_number])[0].card_number
    if (card_number){
